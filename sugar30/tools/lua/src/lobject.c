/*
** $Id: lobject.c,v 1.1.1.1 2002/04/05 08:42:49 strive Exp $
...
/* dbindel: Extension for metric suffixes */
static Number suffix_val(char c)
{
    switch (c) {
...
int luaO_str2d (const char *s, Number *result) {  /* LUA_NUMBER */
  ...
  /* dbindel: Added support for metric suffix extension */
  suffix = suffix_val(*endptr);
  if (suffix) {
    res *= suffix;
    ++endptr;
  }
