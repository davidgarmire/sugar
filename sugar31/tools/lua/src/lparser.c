/*
** $Id: lparser.c,v 1.3 2002/04/17 23:38:14 dbindel Exp $
...
static void subnetbody (LexState *ls, int line);
...
/* dbindel -- 3/9/02 -- added subnet
 * This code was generated by doing a manual trace of the statements
 *
 *  function foo(...) ... end
 *  foo = subnetize(foo)
 */
static void subnetstat (LexState* ls, int line) {
  /* subnetstat -> SUBNET funcname body */
  TString* tsfunc;
  TString* tssub;
  FuncState *fs;
  expdesc vfunc;
  ...
  next(ls);  /* skip SUBNET */
  tsfunc = str_checkname(ls);
  singlevar(ls, tsfunc, &vfunc);
  subnetbody(ls, line);
  luaK_storevar(ls, &vfunc);

  fs = ls->fs;
  tssub = luaS_new(ls->L, "subnetize");
  singlevar(ls, tsfunc, &vfoo);
  singlevar(ls, tssub, &vsub);
  luaK_tostack(ls, &vsub, 1);
  slevel = fs->stacklevel - 1;
  singlevar(ls, tsfunc, &vfoo2);
  luaK_tostack(ls, &vfoo2, 0);
  fs->stacklevel = slevel;
  luaK_code2(fs, OP_CALL, slevel, MULT_RET);
  vsub.k = VEXP;
  vsub.u.l.t = vsub.u.l.f = NO_JUMP;
  luaK_tostack(ls, &vsub, 0);
  adjust_mult_assign(ls, 1, 1);
  luaK_storevar(ls, &vfoo);
  luaK_adjuststack(fs, 0);
}
...
static int stat (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    /* dbindel -- 3/9/02 -- added subnet statement */
    case TK_SUBNET: {  /* stat -> subnetstat */
      subnetstat(ls, line);
      return 0;
    }
...
static void subnetparlist (LexState *ls) {
  /* subnetparlist -> [ param { ',' param } ] */
  int nparam = 0;
  TString* tsargsall;
  TString* tsmaterial;

  /* Function has the signature foo(args) */
  tsargsall = luaS_new(ls->L, "args");
  new_localvar(ls, tsargsall, 0);
  code_params(ls, 1, 0);

  tsmaterial = luaS_new(ls->L, "material");

  /* Actual arguments are retrieved from the args structure */
  if (ls->t.token != ')') {  /* is `parlist' not empty? */
    do {
      FuncState *fs = ls->fs;
      TString* tsarg;
      ...
      switch (ls->t.token) {
        case TK_NAME: 
            ...
            /* local p = ... */
            tsarg = str_checkname(ls);
            new_localvar(ls, tsarg, 0); 

              /* args[n] */
              singlevar(ls, tsargsall, &v1);
              luaK_tostack(ls, &v1, 1);
              v1.k = VINDEXED;
                luaK_number(fs, nparam);
                vnparam.k = VEXP;
                vnparam.u.l.t = vnparam.u.l.f = NO_JUMP;
                luaK_tostack(ls, &vnparam, 1);
              
              /* or */
              luaK_infix(ls, OPR_OR, &v1);

              /* args.p */
              singlevar(ls, tsargsall, &v2);
              luaK_tostack(ls, &v2, 1);
              v2.k = VINDEXED;
                luaK_kstr(ls, string_constant(fs, tsarg));
              
              /* finish or */
              luaK_posfix(ls, OPR_OR, &v1, &v2);

              /* or */
              luaK_infix(ls, OPR_OR, &v1);

              /* (local) material -- nil if there is none */
              singlevar(ls, tsmaterial, &v2);
              if (v2.k == VGLOBAL) {
                  luaK_adjuststack(fs, -1);
                  v2.k = VEXP;
                  v2.u.l.t = v2.u.l.f = NO_JUMP;
              }

              /* and */
              luaK_infix(ls, OPR_AND, &v2);

              /* material.p */
              singlevar(ls, tsmaterial, &v3);
              luaK_tostack(ls, &v3, 1);
              v3.k = VINDEXED;
                luaK_kstr(ls, string_constant(fs, tsarg));

              /* finish and */
              luaK_posfix(ls, OPR_AND, &v2, &v3);

              /* finish or */
              luaK_posfix(ls, OPR_OR, &v1, &v2);

              /* Return expression value */
              luaK_tostack(ls, &v1, 0);
...
static void subnetbody (LexState *ls, int line) {
  ...
  check(ls, ')');
  chunk(ls);
  check_match(ls, TK_END, TK_SUBNET, line);
  close_func(ls);
  pushclosure(ls, &new_fs);
}
